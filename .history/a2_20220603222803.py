...
file_dict = {'test': 's3://comp5349-2022/test.json',
...
# ====================
# Get segment sequences
# Apply user defined function function to convert the sequences into desired types
...
def segment(context):
    ...
    while start < context_length:
        end = start + WINDOW_SIZE if start + WINDOW_SIZE < context_length else context_length
...
selected_df = type_added_df.select("context.*", "text", "id", "question", "title", "label.*")
...
pos_df = selected_df.filter(selected_df.label == "P").cache()
impossible_neg_df = selected_df.filter(selected_df.label == "impossible_N")
possible_neg_df = selected_df.filter(selected_df.label == "possible_N")
pos_df.show(20)
